...
public sealed class EncryptionKeyInitializer : IEncryptionKeyInitializer
{
    ...
    public static string ResolveKeyPath(IConfiguration configuration)
    {
        ...
        {
            return Path.Combine(localAppData, "SftpSync", "encryption.key");
        }

        var home = Environment.GetEnvironmentVariable("HOME");
        if (!string.IsNullOrWhiteSpace(home))
        {
            return Path.Combine(home, ".sftpsync", "encryption.key");
        }

        return Path.Combine(AppContext.BaseDirectory, ".sftpsync", "encryption.key");
    }
