...
function useTheme() {
  const [theme, setTheme] = useState<Theme>(() => (localStorage.getItem("sftpsync:theme") as Theme | null) ?? "system");
...
export function AppShell() {
  ...
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem("sftpsync:sidebar") === "collapsed");
  const [drawerOpen, setDrawerOpen] = useState(false);
  const { theme, setTheme } = useTheme();
  const statusQuery = useQuery({ queryKey: queryKeys.status, queryFn: api.status, refetchInterval: 30_000 });

  useEffect(() => {
    localStorage.setItem("sftpsync:sidebar", collapsed ? "collapsed" : "expanded");
  }, [collapsed]);
  ...
  const themeIcon = theme === "dark" ? <Moon size={16} /> : theme === "light" ? <Sun size={16} /> : <MonitorCog size={16} />;
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      >
        <div className="flex h-14 items-center gap-2 border-b border-border px-3">
          ...
          {!collapsed && <span className="text-sm font-semibold tracking-wide">SftpSync</span>}
        </div>
        ...
        <div className="border-t border-border p-2">
          <Button className="w-full px-2" title={collapsed ? "Expand sidebar" : "Collapse sidebar"} onClick={() => setCollapsed((value) => !value)}>
      ...
      <div className={cn("min-h-screen", collapsed ? "md:pl-16" : "md:pl-64")}>
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-border bg-panel/95 px-4 backdrop-blur">
          <div className="flex min-w-0 items-center gap-3">
            <Button className="md:hidden" title="Open navigation" onClick={() => setDrawerOpen(true)}>
              <Menu size={18} />
            </Button>
            <div>
              <h1 className="truncate text-base font-semibold">{title}</h1>
              <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
                <span className={cn("h-2 w-2 rounded-full", statusQuery.data?.databaseAvailable ? "bg-emerald-500" : "bg-amber-500")} />
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <button className="absolute inset-0 bg-black/45" aria-label="Close navigation" onClick={() => setDrawerOpen(false)} />
          <div className="absolute inset-y-0 left-0 flex w-72 flex-col border-r border-border bg-panel">
            ...
            <nav className="flex flex-col gap-1 p-2" onClick={() => setDrawerOpen(false)}>
              {navItems.map((item) => (
                <NavLink key={item.to} collapsed={false} {...item} />
              ))}
            </nav>
          </div>
        </div>
