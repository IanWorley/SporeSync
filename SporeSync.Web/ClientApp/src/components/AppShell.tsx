...
function useTheme() {
  const [theme, setTheme] = useState<Theme>(
    () => (localStorage.getItem("sporesync:theme") as Theme | null) ?? "system",
  );

  useEffect(() => {
    localStorage.setItem("sporesync:theme", theme);
...
export function AppShell() {
  ...
  const [collapsed, setCollapsed] = useState(
    () => localStorage.getItem("sporesync:sidebar") === "collapsed",
  );
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute inset-y-0 left-0 flex w-72 flex-col border-r border-border bg-panel">
            <div className="flex h-14 items-center gap-2 border-b border-border px-4">
              ...
              <span className="text-sm font-semibold">SporeSync</span>
            </div>
